...
#define PREV_REF 70;
double previousRef = PREV_REF;  // the initial desired reference is PREV_REF [%]
                                // of the effective length of the tube
const double ballDiam = 4;      // diameter of the ball in cm
const int maxWaitTime =
    (int)(columnL * 3.5 / 0.034);  // maximal waiting time for the ultrasonic
                                   // sensors to receive the reflected wave
...
char region =
    'a';  // region 'a', 'b' or 'c' - division of the tube into three regions
// Limits for regions
const int limitAB = 18;  // in % of the effective length of the tube
const int limitBC = 64;  // in % of the effective length of the tube
// Lamp control limits
const int intensLowerLim =
    10;  // The lowest position of the ball that will produce difference in the
         // LED light of the tube
//(for all positions below, there would be no light)
const int intensUpperLim =
    80;  // The highest position of the ball that will produce difference in the
         // LED light of the tube
...
void setup() {
    ...
    pinMode(led, OUTPUT);     // Sets the led pin to an output mode (used for
...
void loop() {
    ...
    if (buttonState == LOW && countButton == 2) {
        ...
        if (flagInstability) {
            delay(8);  // reset needed to let the ball fall down in case of
                       // instability risk
            flagInstability = 0;
        }
        countButton = 2;  // update the indicator of the state
    ...
    if (countButton == 2) {
        ...
        durationH = pulseIn(echoPinHand, HIGH, maxWaitTime);  // Reads the echoPin, returns the sound wave
        ...
        storeHandPos[flagA] =
            distanceH - lowerGap;  // scales the value in the reference system
        ...
        durationB = pulseIn(echoPinBall, HIGH, maxWaitTime);  // Reads the echoPin, returns the sound wave
                                   // travel time in microseconds
        // Calculating the distance
        ...
        if (flagA == 3) {
            ...
            if ((ref < 21) || (ref > 77) || flagRefErr){
                ref = previousRef;
            } 
            flagRefErr = false;
            previousRef = ref;  // stores prevouos value of ref in memory for
                                // next block of 3 iterations
            flagA = 0;  // resets the counter for calculating the average of
                        // reference
        }
        if ((ballPos < 0) || (ballPos > 100) || flagBallPosErr){
            ballPos = prevBallPos;
        }
        ...
        {
            prevControlI = control - Kpb * (ref - ballPos) -
                           Kib * (ref - ballPos) * samplingTime -
                           Kdb * (-ballPos + prevBallPos) /
                               samplingTime;  // bumpless control
            region = 'b';
        } else if (ballPos > limitBC && previousBallPos > limitAB &&
                   previousBallPos <= limitBC)  // transition from region B to C
        {
            prevControlI = control - Kpc * (ref - ballPos) -
                           Kic * (ref - ballPos) * samplingTime -
                           Kdc * (-ballPos + prevBallPos) /
                               samplingTime;  // bumpless control
            region = 'c';
        } else if (ballPos > limitAB && ballPos <= limitBC &&
                   previousBallPos > limitBC)  // transition from region C to B
        {
            prevControlI = control - Kpb * (ref - ballPos) -
                           Kib * (ref - ballPos) * samplingTime -
                           Kdb * (-ballPos + prevBallPos) /
                               samplingTime;  // bumpless control
            region = 'b';
        } else if (ballPos <= limitAB && previousBallPos > limitAB &&
                   previousBallPos <= limitBC)  // transition from region B to A
        {
            prevControlI = control - Kpa * (ref - ballPos) -
                           Kia * (ref - ballPos) * samplingTime -
                           Kda * (-ballPos + prevBallPos) /
                               samplingTime;  // bumpless control
            region = 'a';
        } else if (ballPos > limitBC &&
                   previousBallPos <= limitAB)  // transition from region A to C
        {
            prevControlI = control - Kpc * (ref - ballPos) -
                           Kic * (ref - ballPos) * samplingTime -
                           Kdc * (-ballPos + prevBallPos) /
        ...
        PID(region);  // calculates the control parameters for the current
                      // sampling interval
        // uses the PID to set the value of the power to the motor according to
        // the position and speed
        if (ballPos > 85) {
            motorPower(fanPin, 0);
            // Reset is needed to avoid the ball hit the sensor and remain stick
            // to the sensor
            flagInstability = 1;
            ...
            previousBallPos = ballPos;  // stores prevouos value of ref in
...
double getDistance(int trigPin, int echoPin) {
    // this function get the distance and save the value as output the inputs
    // are the pin of the  US sensor
    unsigned long duration = 0;
    double distance = 0;
    digitalWrite(trigPin, LOW);  // Clears the trigPin
    delayMicroseconds(2);
    digitalWrite(trigPin,
                 HIGH);  // Sets the trigPin on HIGH state for 10 micro seconds
    delayMicroseconds(10);
...
void ledPower(int ledPin, double intensity) {
    // Function that writes the values of the desired intensity on the pin of
    // the tube LED light. The inputs are the pin to wiich the led light is
    // attached and the desired intensity (number 0-80).

    // Scaling the intensity
    if (intensity > intensUpperLim)
        intensity = intensUpperLim;
    else if (intensity < intensLowerLim)
        intensity = intensLowerLim;
...
void PID(char region) {
    ...
    if (control > 100) {  // Technically impossible to actuate, but necessary to
                          // saturate in program...
        control = 100;
...
void printInfoLCD() {
    // Function used to print on the LCD screen all the characters containing
    // the information about the current state of system: Reference level, ball
    // position and applied control, each 0-100 %
    lcd.clear();
    lcd.setCursor(0, 0);
...
void updateInfoLCD() {
    // Function used to print on the LCD screen only the characters that
    // determine the value of important current parameters of the system:
